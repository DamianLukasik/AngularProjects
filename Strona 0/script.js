...
angular.module('myUserList').directive("promiseUser", function($q,$http) {
	var deferred = $q.defer();
	return {
		link: function (scope, element, attrs) {
			element.find("button").on("click", function (event) {//connect to Database MySQL
				var buttonText = event.target.innerText;
				if (buttonText.indexOf("Wyświetl")!=-1){			
					var result;
...
angular.module('myUserList').controller("myCtrl", function($scope, $http) {
	...
	$scope.isHideTab = [];
	$scope.deleteUserMySQL = function (userID){
		$http.get("users_mysql_delete.php?deleteID=" + userID)
            	.then(function onSuccess(response) {
			$scope.isHideTab.push(userID);		
            	})
	}
	$scope.isDeleted = function (id){
		var log = false;
		angular.forEach($scope.isHideTab,function(v){
            		if(v == id){
	                	log = true;
			}
            	});
	...
	$scope.form_newUserMySQL = ['',''];
	$scope.addUserMySQL = function (form){
		$http.get("users_mysql_add.php?name=" + form[0] + "?lastname=" + form[1])
            	.then(function onSuccess(response) {
			alert("dodany");	
            	})
	}
	$scope.editUserMySQL = function (id){
